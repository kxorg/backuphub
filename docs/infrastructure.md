...
Текущая CI/CD-логика, где сборка выполняется на сервере назначения:

![CI/CD до внедрения SecOps](ci-cd-before-secops.png)

Целевая CI/CD-логика после внедрения `secops`-машины, registry и security checks:

![CI/CD после внедрения SecOps](ci-cd-after-secops.png)

Схема GitHub runners и распределения задач между репозиториями:
...
| **Назначение** | Тестовый контур. Сюда попадают изменения из ветки `DEV`; в текущей схеме здесь же выполняется сборка app-образа. | Боевой контур. Принимает реальные данные от backup-инструментов и обслуживает пользователей. | Инфраструктурный контур. Хранит мониторинг, логи, алерты и бэкапы PostgreSQL. | Целевой контур CI/CD и DevSecOps: сборка образов, registry, security scanning и контроль поставки. |
...
`secops` добавлен как отдельный контур развития CI/CD. В текущей рабочей схеме сборка приложения еще выполняется на сервере назначения (`dev`/`prod`), но целевая модель предполагает перенос build, registry и security checks на `secops`.
...
| Node Exporter | `9100` protected | Метрики ОС; не публичный, доступ только Prometheus с `infra` |
| cAdvisor | `8080` protected | Метрики контейнеров; не публичный, доступ только Prometheus с `infra` |
...
`secops` предназначен для развития CI/CD и DevSecOps-практик. Постоянно работающие инфраструктурные сервисы на нем отделяются от задач сборки и сканирования. Registry и security tools добавляются по мере перехода к целевой CI/CD-схеме.

| Контейнер / сервис | Порт | Назначение |
| --- | --- | --- |
| Nginx | `80`, `443` | HTTPS reverse proxy для registry и web-интерфейсов |
| Docker Registry | `5000` internal | Хранилище собранных Docker-образов |
| GitHub Runner | systemd service | Запуск build/deploy/security workflows |
...
| SAST/SCA/DAST tools | on demand | Semgrep, Grype и другие security checks по мере внедрения |
...
```text
Grafana Alloy на dev/prod/infra/secops
...
Эта ручка предназначена только для приема логов от Alloy. Она не является публичным UI. Доступ к ней должен быть ограничен allowlist по IP серверов `dev`, `prod`, `infra` и `secops`.
...
Prometheus на infra
  ...
  -> https://node-exporter.secops.backuphub.spb.ru/metrics
  -> https://cadvisor.secops.backuphub.spb.ru/metrics
  -> node-exporter:9100 на infra
  -> cadvisor:8080 на infra
```

Prometheus работает по pull-модели: он сам забирает метрики с `dev`, `prod`, `infra` и `secops`.

На `dev`, `prod` и `secops` Node Exporter и cAdvisor не должны быть публично доступны. Метрики отдаются через отдельные protected endpoints для Prometheus; доступ ограничивается Nginx allowlist и/или firewall/iptables по IP `infra`, где работает Prometheus.
...
| Endpoint | Куда проксирует | Доступ |
| --- | --- | --- |
| `https://registry.backuphub.spb.ru` | `registry:5000` | Целевая модель; только CI/CD runners и серверы deploy |
...
CI/CD описывается в двух состояниях: текущая рабочая схема и целевая схема после внедрения `secops`.

В текущей рабочей схеме Docker Registry еще не участвует в поставке приложения. Образ собирается прямо на том сервере, где должен запускаться контейнер.

В целевой схеме сборка, security scanning и push образов будут вынесены на `secops`. `dev` и `prod` будут забирать уже готовый image из `registry.backuphub.spb.ru`.
...
Проверки PR выполняются на GitHub-hosted runner. Если обязательные тесты не проходят, merge в защищенную ветку `DEV` блокируется. Для изменений только в документации workflow может быть пропущен отдельным `paths`/`paths-ignore` правилом.
...
Тег `<commit_sha>` нужен для точного понимания, какая версия кода запущена, и для возможности отката. Тег `dev-latest` удобен как указатель на последнюю успешную сборку ветки `DEV`, но для deploy предпочтительно фиксировать конкретный commit SHA.
...
| `secops` | `80`, `443` | HTTPS reverse proxy для registry и SecOps web-интерфейсов |
| `dev/prod/infra/secops` | `8228` | SSH только для админов |
...
| `9100` | Node Exporter | Не публичный; доступ только Prometheus с `infra` через protected endpoint / firewall allowlist |
| `8080` | cAdvisor | Не публичный; доступ только Prometheus с `infra` через protected endpoint / firewall allowlist |
...
| `5000` | Docker Registry | Не публикуется напрямую; доступ через `registry.backuphub.spb.ru` |
...
В интернет можно открывать только минимально необходимые порты. Для `dev`, `prod`, `infra` и `secops` публичны только `80`, `443` и `8228` для администраторов.
...
Для `dev`, `prod`, `infra` и `secops` должны использоваться разные секреты.
...
Для контейнерного мониторинга используется Grafana dashboard на основе cAdvisor. Основные фильтры: `node`, `job`, `service`, `container`. В фильтрах должны быть доступны все серверы: `dev`, `prod`, `infra`, `secops`.

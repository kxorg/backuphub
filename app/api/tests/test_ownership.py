...
class TestOwnershipIsolation:
    def test_list_returns_only_own_operations(self, api_client):
        ...
        BackupOperationFactory(backup_configuration_version=cv_a, status='success')
        BackupOperationFactory(backup_configuration_version=cv_a, status='success')
        BackupOperationFactory(backup_configuration_version=cv_b, status='success')
    ...
    def test_cannot_update_foreign_operation(self, api_client):
        ...
        assert resp.status_code == 404  # get_object() не найдёт — чужой